...
public class QuotaManager : NetworkSingleton<QuotaManager>
{
    ...
    private NetworkVariable<float> CurrentQuota = new NetworkVariable<float>(0, NetworkVariableReadPermission.Everyone,
    ...
    private NetworkVariable<int> CurrentDayOfQuota = new NetworkVariable<int>(0, NetworkVariableReadPermission.Everyone,
        NetworkVariableWritePermission.Server);
    ...
    public void HandleEnterHub(OnEnterHubEvent onEnterHubEvent)
    {
        ...
        if (!IsServer) return;
        if (onEnterHubEvent.BeforeNewRun)
        {
            HandleNewQuota(true);
            return;
        }
